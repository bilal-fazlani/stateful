...
class Account(accountNumber: Int, ledger: Ledger)(implicit singleThreadedEc: ExecutionContext, mat: Materializer) {
  //public vars only for demo/debugging
  var balance = 0
  ...
  def deposit(amount: Int): Future[Unit] = Future.unit.flatMap { _ =>
    ...
    ledger.record(deposit)(singleThreadedEc).map { x =>
      balance += amount
      actions ::= deposit
      queue.offer(deposit)
    }
  ...
  def withdraw(amount: Int): Future[Unit] = Future.unit.flatMap { _ =>
    ...
    ledger.record(withdrawal)(singleThreadedEc).map { _ =>
      balance -= amount
      actions ::= withdrawal
